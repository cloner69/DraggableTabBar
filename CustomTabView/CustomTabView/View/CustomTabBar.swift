    ...
    var body: some View {
        ...
        .onChange(of: properties.moveLocation) { oldValue, newValue in
            if let droppingIndex = properties.tabs.firstIndex(where: {
                $0.rect.contains(newValue)}), let activeIndex = properties.tabs.firstIndex(where: { $0.id == properties.movingTab }), droppingIndex != activeIndex {
                /// Swapping animation
...
struct TabBarButton: View {
    ...
    var body: some View {
        ...
        Image(systemName: tab.symbolImage)
            ...
            .padding(.vertical, 12)
            .foregroundStyle(properties.activeTab == tab.id ? .primary : .secondary)
                            ...
                            CustomGesture(isEnabled: $binding.editMode, trigger: { status in
                                if status {
                                    ...
                                    withAnimation(.easeInOut(duration: 0.3), completionCriteria: .logicallyComplete) {
                                        /// Finishing with the updated location (happends when items swapped betweeen one another)
                                        properties.initialTabLocation = tabRect
